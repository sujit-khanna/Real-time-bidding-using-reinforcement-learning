from env import rtb_environment_cnn as rtb_env
import pandas as pd
import numpy as np
...
ENV_ID = "RTBEnv-v0"
GAMMA = 0.99
REWARD_STEPS = 60 #how many steps ahead bellman's equation is unrolled to estimate discounted total reward of every transaction
BATCH_SIZE = 1
LEARNING_RATE = 1e-4
ENTROPY_BETA = 1e-3
...
if __name__ == "__main__":
    ...
    model_type = f" 1.25x Budget CNN model unique budget tanh action -a2c_l_r: {LEARNING_RATE}, en_b :{ENTROPY_BETA}, " \
                 f"max_ep:{MAX_EPISODES}, max_it: {TEST_ITERS}, reward_steps: {REWARD_STEPS}  batch_size: {BATCH_SIZE}"
    ...
    writer = SummaryWriter(comment=f" 1.25x Budget CNN model unique budget tanh action -a2c_l_r: {LEARNING_RATE}, en_b :{ENTROPY_BETA}, max_ep:"
