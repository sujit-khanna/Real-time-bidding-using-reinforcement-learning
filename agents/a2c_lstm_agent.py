from env import rtb_environment_rnn as rtb_env
import os
import time
import math
import ptan
...
ENV_ID = "RTBEnv-v0"
GAMMA = 0.99
REWARD_STEPS = 12 #how many steps ahead bellman's equation is unrolled to estimate discounted total reward of every transaction
BATCH_SIZE = 16
LEARNING_RATE = 1e-4
ENTROPY_BETA = 1e-2
...
def test_net(net, env, count=10, device="cpu"):
    ...
    for _ in range(count):
        ...
        while True:
            ...
            steps += 1
            if done:
                mean_actions = test_actions/steps
                break
    return rewards / count, steps / count, mean_actions / count
...
if __name__ == "__main__":
    ...
    model_type = f" LSTM RNN unique budget tanh action -a2c_l_r: {LEARNING_RATE}, en_b :{ENTROPY_BETA}, max_ep: " \
                 f"{MAX_EPISODES}, max_it: {TEST_ITERS}, reward_steps: {REWARD_STEPS}  batch_size: {BATCH_SIZE}"
    ...
    writer = SummaryWriter(comment=f" LSTM RNN unique budget tanh action -a2c_l_r: {LEARNING_RATE}, en_b :{ENTROPY_BETA}, max_ep:"
                                   f" {MAX_EPISODES}, max_it: {TEST_ITERS}, reward_steps: "
                                   f"{REWARD_STEPS}  batch_size: {BATCH_SIZE}" + SAVEPATH)
        ...
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            for step_idx, exp in enumerate(exp_source):
                ...
                if step_idx % TEST_ITERS == 0:
                    ts = time.time()
                    rewards, steps, actions = test_net(net, test_env, device=device)
                    print("Test done is %.2f sec, reward %.3f, steps %d" % (
                        time.time() - ts, rewards, steps))
                    writer.add_scalar("test_reward", rewards, step_idx)
                    writer.add_scalar("test_steps", steps, step_idx)
                    writer.add_scalar("test_actions", actions, step_idx)

                    if best_reward is None or best_reward < rewards:
                        if best_reward is not None:
                            print("Best reward updated: %.3f -> %.3f" % (best_reward, rewards))
                            name = "model_%sbest_%+.3f_%d.dat" % (model_type,rewards, step_idx)
                ...
                loss_value_v = F.mse_loss(value_v.squeeze(-1), vals_ref_v.squeeze(-1)) #critic loss
