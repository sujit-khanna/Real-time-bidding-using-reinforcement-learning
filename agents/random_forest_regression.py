...
MAX_EPISODES=2500
TRAINED_MODEL = "../saved_models/CB_bid_price_regression_trunc2"
# TRAINED_MODEL = "../saved_models/RF_bid_price_regression_trunc.pkl"
ENV_ID = "RTBEnv-v0"
GAMMA = 0.99
...
MODEL = CatBoostRegressor().load_model(TRAINED_MODEL)
...
def test_net(net, env, count=10, device="cpu"):
    ...
    for _ in range(count):
        ...
        while True:
            ...
            action = MODEL.predict(obs[:-1])
            # action = MODEL.predict(obs[:-1].T.reshape(1, -1))
...
if __name__ == "__main__":
    ...
    env = rtb_env.RTBEnvironment.load_from_h5(STORE_LOCATION, TABLE_NAME, external_model=False)
    test_env = rtb_env.RTBEnvironment.load_from_h5(STORE_LOCATION, TABLE_NAME, external_model=True)

    net = model.ModelA2C(env.observation_space.shape[0], env.action_space.shape[0]).to(device)
    print(net)

    writer = SummaryWriter(comment="-CB_regression_-unique budget" + SAVEPATH)
