...
def train_regression_model(df, feature_list, target_list,model_type="RF"):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, train_size=0.95, test_size=0.05, random_state=4)
    if model_type=="RF":
        model = RandomForestRegressor(max_depth=2, random_state=0)
        model.fit(X_train.values, y_train.values)
        joblib.dump(model, "../saved_models/RF_full_bid_price_regression_trunc.pkl")
...
if __name__ == '__main__':
    ...
    df = load_data()
    train_regression_model(df, feature_list, target_value, model_type="RF")
