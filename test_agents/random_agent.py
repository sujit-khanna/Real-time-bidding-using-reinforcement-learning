...
def run_random_agent():
    ...
    for i in range(EPISODES):
        ...
        while not done:
            ...
            print(f"Episode: {i}, TimeStamp: {info['timestamp']}, Reward:{reward}, remaining budget (%): "
                  f"{(info['remaining budget']/MAX_DAILY_BUDGET)*100}")
    ...
    pd.DataFrame({"Episodes": episode_list, "rewards": reward_list, "rem_budget":budget_list}).to_csv(f"{RESULTS_PATH}random_5min_agent_testset.csv")
