...
class RTBEnvironment(gym.Env):
    ...
    def __init__(self, df, external_model=False, reset_budget=True):
        self.df = df
        self.df = self.df[self.df["episode_key"]!="2821_6.0"]
        ...
        self.observation_space = spaces.Box(low=0.0, high=1.0, shape=(7 * STATE_LOOKBACK,), dtype=np.float)
    ...
    def reset(self):
        self.episode_day = self.df.episode_key.unique().tolist()
        self.episode_df = self.df.loc[self.df["episode_key"] == self.episode_day[self.ep_count]]
        print(self.episode_df["episode_key"].iloc[0])
        self.episode_df.index = list(range(self.episode_df.shape[0]))
        # self.budget = MAX_DAILY_BUDGET # change budget here
        self.max_budget = self.budget = self.episode_df["daily_budget"].iloc[0]
        self.total_reward = 0
